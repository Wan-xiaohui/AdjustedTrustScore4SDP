...
def bm_test(x, y, ttype=0, alpha=0.05):
    ...
    denominator1 = ((N_x + N_y) * math.sqrt(N_x * S2_x + N_y * S2_y))
    if denominator1 == 0:
        W = float('nan')  # or some other appropriate value or handling
    else:
        W = ((N_x * N_y) * (Ravr_y - Ravr_x)) / denominator1
    ...
    if f_hat_den == 0:
        f_hat = float('nan')  # or some other appropriate value or handling
    else:
