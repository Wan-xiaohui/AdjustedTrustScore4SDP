    ...
    if is_correct:
        for clf_index in range(8):
            ...
            for n in range(len(fname)):
                ...
                result = pd.read_csv('dump/csvs/RQ2/Correct/' + fname[n] + ".csv")
                result.insert(0, 'dataset', os.path.splitext(fname[n])[0])
                final_results = pd.concat([final_results, result.iloc[clf_index, [3, 4, 2]]], axis=1)

            final_results = final_results.T
            final_results.insert(0, 'dataset', [fname[i] for i in range(len(fname))])
            final_results.to_csv('dump/csvs/RQ2/results_correct_{}.csv'.format(clf), index=False)
        ...
        for clf_index in range(8):
            ...
            for n in range(len(fname)):
                ...
                result = pd.read_csv('dump/csvs/RQ2/Incorrect/' + fname[n] + ".csv")
                result.insert(0, 'dataset', os.path.splitext(fname[n])[0])
                final_results = pd.concat([final_results, result.iloc[clf_index, [3, 4, 2]]], axis=1)

            final_results = final_results.T
            final_results.insert(0, 'dataset', [fname[i] for i in range(len(fname))])
            final_results.to_csv('dump/csvs/RQ2/results_incorrect_{}.csv'.format(clf), index=False)
...
def total_analysis(is_correct=True):
    ...
    if is_correct:
        for clf_index in range(8):
            ...
            current_classifier_data = pd.read_csv('dump/csvs/RQ2/results_correct_{}.csv'.format(clf))
        ...
        total_results.to_csv('dump/csvs/RQ2/total_results_correct.csv', index=False)
        ...
        for clf_index in range(8):
            ...
            # Read the CSV file for the current classifier
            current_classifier_data = pd.read_csv('dump/csvs/RQ2/results_incorrect_{}.csv'.format(clf))
        ...
        # Save the total_results DataFrame to a CSV file
        total_results.to_csv('dump/csvs/RQ2/total_results_incorrect.csv', index=False)
